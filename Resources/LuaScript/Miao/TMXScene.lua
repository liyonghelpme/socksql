...
function TMXScene:ctor()
    ...
    sp1:runAction(repeatForever(sequence({scaleto(0.5, 10.1, 10.1), scaleto(0.5, 10, 10)})))

    local sp1 = CCSprite:create("light.png")
    addChild(self.cameraLight, sp1)
    sp1:setBlendFunc(bf)
    setScale(sp1, 8)
    setAnchor(setPos(sp1, {10, 200}), {0, 0.5})
    setRotation(sp1, -33)
    setColor(sp1, {212/3, 140/3, 36/3})



    --[[
    setRotation(sp1, 33)
    local len = distance({0, 0}, {vs.width, vs.height})
    local plen = sp1:getContentSize()
    plen = distance({0, 0}, {plen.width, plen.height})
    setScaleX(sp1, len/plen)
    --]]

    --[[
    print("add Camera Light #####################")
    print("add Camera Light #####################")
    print("add Camera Light #####################")
    print("add Camera Light #####################")
    print("add Camera Light #####################")
